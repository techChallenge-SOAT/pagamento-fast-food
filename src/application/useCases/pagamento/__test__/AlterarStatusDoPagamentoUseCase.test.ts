...
describe('AlterarStatusDoPagamentoUseCase', () => {
  ...
  it('deve lançar um erro se o status atual não for "aguardando"', async () => {
    ...
    await expect(AlterarStatusDoPagamentoUseCase.execute('123', Status.Pago)).rejects.toThrow(
  ...
  it('deve lançar um erro se o novo status for inválido', async () => {
    (PagamentoRepository.buscarPorId as jest.Mock).mockResolvedValue(mockPedido);

    await expect(AlterarStatusDoPagamentoUseCase.execute('123', 'invalido' as Status)).rejects.toThrow(
      'Status inválido',
    );
  });
  ...
  it('deve atualizar o status do pagamento e retornar os detalhes do pedido atualizados', async () => {
    (PagamentoRepository.buscarPorId as jest.Mock).mockResolvedValue(mockPedido);
    (PedidoFastFoodService.consultarPedido as jest.Mock).mockResolvedValue(mockDetalhePedido);

    const result = await AlterarStatusDoPagamentoUseCase.execute('123', Status.Pago);

    expect(PagamentoRepository.atualizarStatus).toHaveBeenCalledWith('123', Status.Pago);
    expect(PedidoFastFoodService.consultarPedido).toHaveBeenCalledWith('pedido_123');
  ...
  it('deve atualizar o status do pagamento para cancelado', async () => {
    ...
    const result = await AlterarStatusDoPagamentoUseCase.execute('123', Status.Cancelado);

    expect(PagamentoRepository.atualizarStatus).toHaveBeenCalledWith('123', Status.Cancelado);
    expect(PedidoFastFoodService.consultarPedido).toHaveBeenCalledWith('pedido_123');
    expect(result).toEqual({
      ...mockDetalhePedido.data,
