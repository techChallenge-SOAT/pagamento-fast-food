...
function gerarPixPayload(pixData: DadosPagamento): string {
    ...
    const idEndToEnd: string = `E${String(dataAtual.getDate()).padStart(2, '0')}${String(dataAtual.getMonth() + 1).padStart(2, '0')}${dataAtual.getFullYear()}${String(dataAtual.getHours()).padStart(2, '0')}${String(dataAtual.getMinutes()).padStart(2, '0')}${String(dataAtual.getSeconds()).padStart(2, '0')}${gerarStringAleatoria()}`;
...
export async function gerarQrCodePix({ valor, descricao_pedido, cpf_cliente, id }: DadosPagamento): Promise<{ emv: string, img: string, id_pagamento: string }> {
    const payload: string = gerarPixPayload({ valor, descricao_pedido, cpf_cliente, id });
